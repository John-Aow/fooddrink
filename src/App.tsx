...
import type { CartItem, Locale, MenuItem, Order, OrderStatus } from './types'
import './App.css'

const t = (locale: Locale, th: string, en: string) => locale === 'th' ? th : en
const money = (value: number) => `฿${value.toLocaleString('th-TH')}`
const statusText = (status: OrderStatus, locale: Locale) => ({ pending: t(locale, 'รอรับออเดอร์', 'Waiting'), preparing: t(locale, 'กำลังเตรียม', 'Preparing'), served: t(locale, 'เสิร์ฟแล้ว', 'Served'), cancelled: t(locale, 'ยกเลิก', 'Cancelled') }[status])
...
function App() {
  ...
  const submitOrder = () => { if (!cart.length || session.status === 'closed') return; createOrder(sessionId, cart); setCart([]); setOrders(getOrders(sessionId)); setConfirming(false); setFeedback(t(locale, 'ส่งออเดอร์เรียบร้อยแล้ว', 'Order sent successfully')); setView('menu'); window.setTimeout(() => setFeedback(''), 3000) }

  if (session.status === 'closed') return <div className="closed"><div className="closed-mark">×</div><h1>{t(locale, 'โต๊ะนี้ปิดแล้ว', 'This table is closed')}</h1></div>

  return <div className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-mark">F</div><div><strong>FOOD<span>DRINK</span></strong><small>{t(locale, 'สั่งอาหารที่โต๊ะ', 'Dine-in ordering')}</small></div></div><div className="header-actions"><div className="table-pill">{t(locale, 'โต๊ะ', 'Table')} {session.tableNumber}</div><button className="language" onClick={() => setLocale(locale === 'th' ? 'en' : 'th')}>{locale === 'th' ? 'EN' : 'TH'}</button></div></header>
    <main>
      <div className="page-heading"><div><p className="eyebrow">{t(locale, 'ยินดีต้อนรับ', 'WELCOME')}</p><h1>{t(locale, 'เลือกเมนูที่คุณชอบ', 'Choose your favorites')}</h1></div><div className="live-dot"><i /> {t(locale, 'โต๊ะพร้อมสั่ง', 'Table is open')}</div></div>
    ...
    {cartCount > 0 && <button className="cart-bar" onClick={() => setView('cart')}><span className="cart-icon">🛒</span><span>{t(locale, 'ดูตะกร้า', 'View cart')} <b>{cartCount}</b></span><strong>{money(cartTotal)}</strong><span>›</span></button>}
    <nav className="bottom-nav"><button className={view === 'menu' ? 'selected' : ''} onClick={() => setView('menu')}><span>▦</span>{t(locale, 'เมนูอาหาร', 'Menu')}</button><button className={view === 'cart' ? 'selected' : ''} onClick={() => setView('cart')}><span>🛒{cartCount > 0 && <em>{cartCount}</em>}</span>{t(locale, 'ตะกร้า', 'Cart')}</button><button className={view === 'orders' ? 'selected' : ''} onClick={() => setView('orders')}><span>☷</span>{t(locale, 'ออเดอร์ของโต๊ะ', 'Table orders')}</button></nav>
    {noteId && <div className="modal-backdrop"><div className="modal"><button className="close" onClick={() => setNoteId(null)}>×</button><h2>{t(locale, 'เพิ่มหมายเหตุ', 'Add a note')}</h2><p>{t(locale, 'แจ้งรายละเอียดเพิ่มเติมสำหรับเมนูนี้', 'Add a special request for this item')}</p><textarea value={note} onChange={(event) => setNote(event.target.value)} maxLength={200} placeholder={t(locale, 'เช่น ไม่ใส่ผักชี', 'e.g. No coriander')} /><button className="primary full" onClick={saveNote}>{t(locale, 'บันทึก', 'Save note')}</button></div></div>}
    {confirming && <div className="modal-backdrop"><div className="modal confirm"><button className="close" onClick={() => setConfirming(false)}>×</button><h2>{t(locale, 'ยืนยันการสั่งอาหาร', 'Confirm your order')}</h2><p>{t(locale, 'กรุณาตรวจสอบรายการก่อนยืนยัน เมื่อส่งแล้วจะไม่สามารถแก้ไขหรือลบได้', 'Please review your order. Once sent, it cannot be edited or deleted.')}</p><div className="confirm-total"><span>{t(locale, 'ยอดรวม', 'Total')}</span><strong>{money(cartTotal)}</strong></div><button className="primary full" onClick={submitOrder}>{t(locale, 'ยืนยันส่งออเดอร์', 'Send order')}</button><button className="secondary full" onClick={() => setConfirming(false)}>{t(locale, 'กลับไปแก้ไข', 'Back to edit')}</button></div></div>}
  </div>
}

function MenuCard({ item, locale, quantity, onAdd, onChange }: { item: MenuItem; locale: Locale; quantity: number; onAdd: () => void; onChange: (amount: number) => void }) {
  return <article className={`menu-card ${!item.available ? 'unavailable' : ''}`}><div className="food-image"><img src={item.imageUrl} alt="" />{!item.available && <span>{locale === 'th' ? 'หมดชั่วคราว' : 'Unavailable'}</span>}</div><div className="menu-info"><h3>{locale === 'th' ? item.nameTh : item.nameEn}</h3><p>{locale === 'th' ? item.descriptionTh : item.descriptionEn}</p><div className="card-bottom"><strong>{money(item.price)}</strong>{quantity === 0 ? <button disabled={!item.available} className="add-button" onClick={onAdd}>+</button> : <div className="quantity"><button onClick={() => onChange(-1)}>−</button><b>{quantity}</b><button onClick={() => onChange(1)}>+</button></div>}</div></div></article>
}
...
function EmptyState({ text }: { locale: Locale; text: string }) { return <div className="empty"><span>🍽</span><p>{text}</p></div> }
export default App
